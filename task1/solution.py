...
class Model(object):
    ...
    def __init__(self):
        ...
        kernel = Matern(length_scale=1.5, nu=1.5) + RationalQuadratic(
            alpha=0.7, length_scale=1.1
        ) * DotProduct(sigma_0=0.3)
        self.gps = [GaussianProcessRegressor(kernel=kernel) for _ in range(NUM_MODELS)]
        self.x_median = None
        self.y_median = None
    ...
    def make_predictions(
        ...
        gp_means = np.zeros(test_x_2D.shape[0], dtype=float)
        gp_stds = np.zeros(test_x_2D.shape[0], dtype=float)

        for idx, testPoint in enumerate(test_x_2D):
            testPoint = testPoint.reshape(1, -1)
            gp_mean = None
            gp_std = None

            if testPoint[0][0] < self.x_median and testPoint[0][1] < self.y_median:
                gp_mean, gp_std = self.gps[0].predict(testPoint, return_std=True)  # type: ignore
            elif testPoint[0][0] >= self.x_median and testPoint[0][1] < self.y_median:
                gp_mean, gp_std = self.gps[1].predict(testPoint, return_std=True)  # type: ignore
            elif testPoint[0][0] < self.x_median and testPoint[0][1] >= self.y_median:
                gp_mean, gp_std = self.gps[2].predict(testPoint, return_std=True)  # type: ignore
            elif testPoint[0][0] >= self.x_median and testPoint[0][1] >= self.y_median:
                gp_mean, gp_std = self.gps[3].predict(testPoint, return_std=True)  # type: ignore

            gp_means[idx] = gp_mean
            gp_stds[idx] = gp_std

        predictions = np.zeros(test_x_2D.shape[0], dtype=float)

        idx_array = test_x_AREA.astype(bool)

        predictions[idx_array] = gp_means[idx_array] + 1.75 * gp_stds[idx_array]
        predictions[~idx_array] = gp_means[~idx_array]
    ...
    def fitting_model(self, train_y: np.ndarray, train_x_2D: np.ndarray):
        ...
        """
        data_partitions = self.partition_data_with_median(train_x_2D, train_y)

        for i, (partition_X, partition_y) in enumerate(data_partitions):
            self.gps[i].fit(partition_X, partition_y)

    def partition_data_with_median(self, X, y):
        self.x_median = np.median(X[:, 0])
        self.y_median = np.median(X[:, 1])

        condition1 = (X[:, 0] < self.x_median) & (X[:, 1] < self.y_median)
        condition2 = (X[:, 0] >= self.x_median) & (X[:, 1] < self.y_median)
        condition3 = (X[:, 0] < self.x_median) & (X[:, 1] >= self.y_median)
        condition4 = (X[:, 0] >= self.x_median) & (X[:, 1] >= self.y_median)

        set1, set1_y = X[condition1], y[condition1]
        set2, set2_y = X[condition2], y[condition2]
        set3, set3_y = X[condition3], y[condition3]
        set4, set4_y = X[condition4], y[condition4]

        return [(set1, set1_y), (set2, set2_y), (set3, set3_y), (set4, set4_y)]
