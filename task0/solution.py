...
HYPOTHESIS_SPACE = [
    norm(loc=0.0, scale=math.sqrt(VARIANCE)),
...
def generate_sample(n_samples, seed=None):
    ...
    print("Hypothesis idx: %d" % hypothesis_idx)
...
def log_posterior_probs(x):
    ...
    log_prior_normal = np.log(0.35)
    log_prior_laplace = np.log(0.25)
    log_prior_t = np.log(0.4)

    log_likelihood_normal = np.sum(norm.logpdf(x, 0, normal_scale))
    log_likelihood_laplace = np.sum(laplace.logpdf(x, 0, laplace_scale))
    log_likelihood_t = np.sum(t.logpdf(x, student_t_df))

    log_evidence = logsumexp(
        [
            log_likelihood_normal + log_prior_normal,
            log_likelihood_laplace + log_prior_laplace,
            log_likelihood_t + log_prior_t,
        ]
    )

    # Log-Posterior for Normal distribution
    log_posterior_normal = log_likelihood_normal + log_prior_normal - log_evidence

    # Log-Posterior for Laplace distribution
    log_posterior_laplace = log_likelihood_laplace + log_prior_laplace - log_evidence

    # Log-Posterior for Student's t-distribution
    log_posterior_t = log_likelihood_t + log_prior_t - log_evidence

    log_p = np.array([log_posterior_normal, log_posterior_laplace, log_posterior_t])
