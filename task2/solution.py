...
def main():
    ...
    # to save computational time when developing tche task
...
class SWAGInference(object):
    ...
    def __init__(
        ...
        # SWAG-diagonal
        self.weight_means = self._create_weight_copy()
        self.weight_vars = self._create_weight_copy()
        self.swag_step = 0

        # Full SWAG
        self.deviation_vectors = {}
        for name, _ in self.network.named_parameters():
            self.deviation_vectors[name] = deque(maxlen=self.deviation_matrix_max_rank)
    ...
    def update_swag(self) -> None:
        ...
        # SWAG-diagonal (we also want to do this if we are doing full SWAG, since we need the diagonal part)
        for name, param in current_params.items():
            old_mean = self.weight_means[name]
            old_var = self.weight_vars[name]

            # Update mean
            new_mean = (old_mean * (self.swag_step - 1) + param) / self.swag_step
            self.weight_means[name] = new_mean

            # Update variance
            new_var = (old_var * (self.swag_step - 1) + param ** 2) / self.swag_step
            self.weight_vars[name] = new_var
            

            # Full SWAG
            if self.inference_mode == InferenceMode.SWAG_FULL:
                # Compute the deviation vector for the current parameter
                deviation_vector = (param - self.weight_means[name]).flatten()
                
                # Append the new deviation to the deque
                self.deviation_vectors[name].append(deviation_vector)
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        self.swag_step = 0  # Resetting the SWAG step counter
    ...
    def own_ece(self, predicted_probabilities: torch.Tensor, labels: torch.Tensor, n_bins: int = 30):
        ...
        if torch.min(labels) == -1:
            ...
            labels = torch.where(labels == -1, torch.full_like(labels, num_classes), labels)
            num_classes += 1
            predicted_probabilities = torch.cat((predicted_probabilities, torch.zeros((num_samples, 1), device=predicted_probabilities.device)), axis=1)

        # Predictions are the classes with highest probability
        predictions = torch.argmax(predicted_probabilities, axis=1)
        prediction_confidences = predicted_probabilities[torch.arange(num_samples), predictions]

        # Use uniform bins on the range of probabilities, i.e. closed interval [0.,1.]
        bin_upper_edges = torch.linspace(0, 1, steps=n_bins+1)[1:]

        probs_as_bin_num = torch.bucketize(prediction_confidences, bin_upper_edges, right=True)
        sums_per_bin = torch.zeros(n_bins, device=predicted_probabilities.device)
        sums_per_bin.index_add_(0, probs_as_bin_num - 1, prediction_confidences)

        total_per_bin = torch.bincount(probs_as_bin_num - 1, minlength=n_bins).float()
        total_per_bin = total_per_bin + torch.finfo(sums_per_bin.dtype).eps  # Avoid division by zero

        avg_prob_per_bin = sums_per_bin / total_per_bin

        onehot_labels = torch.nn.functional.one_hot(labels, num_classes=num_classes)
        accuracies = onehot_labels[torch.arange(num_samples), predictions].float()  # accuracies[i] is 0 or 1
        accuracies_per_bin = torch.zeros(n_bins, device=predicted_probabilities.device)
        accuracies_per_bin.index_add_(0, probs_as_bin_num - 1, accuracies) / total_per_bin

        prob_of_being_in_a_bin = total_per_bin / float(num_samples)

        ece_ret = torch.abs(accuracies_per_bin - avg_prob_per_bin) * prob_of_being_in_a_bin
        ece_ret = torch.sum(ece_ret)
        return ece_ret
    ...
    def calibrate_temperature(self, validation_loader):
        ...
        def closure():
            optimizer.zero_grad()
            loss = 0.0
            for inputs, _, _, labels in validation_loader:
                logits = self.network(inputs)
                scaled_logits = logits / self.temperature
                probs = softmax(scaled_logits, dim=1)
                loss += self.own_ece(probs, labels)
            loss.backward()
            return loss
    ...
    def calibrate(self, validation_data: torch.utils.data.Dataset) -> None:
        ...
        val_loader = torch.utils.data.DataLoader(validation_data, batch_size=32, shuffle=False)
        correct_confidences = []
        
        self.network.eval()  # Set the network to evaluation mode
        with torch.no_grad():
            for val_xs, val_is_snow, val_is_cloud, val_ys in val_loader:
                pred_ys = self.network(val_xs)
                pred_probs = torch.softmax(pred_ys, dim=-1)
                pred_labels = pred_probs.argmax(dim=-1)
                
                mask = (pred_labels == val_ys) & (val_ys != -1)  # Correct and well-defined samples
                correct_confidences.extend(pred_probs[mask].max(dim=-1).values.tolist())
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            with torch.no_grad():
                for (batch_xs,) in loader:
                    pred_ys = self.network(batch_xs)
                    scaled_pred_ys = pred_ys #/ self.temperature
                    pred_probs = torch.softmax(scaled_pred_ys, dim=-1)
                    model_sample_predictions.append(pred_probs)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_mean = self.weight_means[name]

            current_var = self.weight_vars[name] - (current_mean ** 2)
            ...
            # Diagonal part
            sampled_param = current_mean + (1/torch.sqrt(torch.tensor(2.))) * current_std * z_1


            # Full SWAG part
            if self.inference_mode == InferenceMode.SWAG_FULL:
                # If we have run enough iterations then this will be equal to deviation_matrix_rank, however it could be less
                current_size = len(self.deviation_vectors[name])

                z_2 = torch.randn(current_size)
                D_hat = torch.stack(list(self.deviation_vectors[name]), dim=0)
                D_hat_sample = D_hat.t() @ z_2 / torch.sqrt(torch.tensor(2*(current_size-1)))

                reshaped_D_hat_sample = D_hat_sample.view(current_mean.size())

                assert reshaped_D_hat_sample.size() == param.size()

                #Combine with diagonal part
                sampled_param += reshaped_D_hat_sample
    ...
    def entropy(self, probs):
        return -np.sum(probs * np.log2(probs + 1e-9))  # small value added to avoid log(0)
